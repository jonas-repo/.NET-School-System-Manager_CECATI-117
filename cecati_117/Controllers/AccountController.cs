...
namespace cecati_117.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    if (string.IsNullOrEmpty(returnUrl))
                    {
                        ...
                        var usuarioLogeado = UserManager.FindByEmail(model.Email);

                        var rol = usuarioLogeado.Roles.SingleOrDefault().RoleId;
                        var rol_usuario = context.Roles.SingleOrDefault(r => r.Id == rol).Name;

                       
                        if(rol_usuario.Equals("Admin"))
                        {
                            return RedirectToLocal("~/Admin/PaginaInicio/Index");
                        }
                        else
                        {
                            if (rol_usuario.Equals("Alumno") || rol_usuario.Equals("Maestro"))
                            {
                                return RedirectToLocal("~/Users/PaginaInicioUsers/Index");
                            }
                            else
                            {
                ...
                default:
                    ModelState.AddModelError("", "Intento de inicio de sesión no válido verifica tu usuario y contraseña.");
                    return View(model);
            }
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model , string[] nombre,string numero_control)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email , FullName = model.FullName + " "+ nombre[0] + " " + nombre[1] };
